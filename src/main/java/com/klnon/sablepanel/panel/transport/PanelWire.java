...
final class PanelWire {
    ...
    private static PanelFrame response(long id, PanelResponse response, boolean allowCompression) throws IOException {
        ...
        meta.addProperty("compressible", capped.compressible());
        JsonObject headers = new JsonObject();
        capped.headers().forEach(headers::addProperty);
        meta.add("headers", headers);
    ...
    /**
     * 响应出口的唯一收口:null 兜底 500;{@link #response} 同步抛(超帧长)时断连。
     * codec 那道 meta 闸是异步 promise 失败,这里的 catch 看不见——响应会被丢弃、
     * 请求方等到超时。今天不可达:响应 meta 只有 status/content_type/gzip/headers,构造侧有界。
     */
    ...
    static PanelResponse response(PanelFrame frame) throws IOException {
        ...
        if (meta.has("headers") && meta.get("headers").isJsonObject()) {
            meta.getAsJsonObject("headers").entrySet().forEach(entry -> headers.put(entry.getKey(), entry.getValue().getAsString()));
        }
        return new PanelResponse(meta.get("status").getAsInt(), string(meta, "content_type"), body,
                meta.has("compressible") && meta.get("compressible").getAsBoolean(), headers);
    }
