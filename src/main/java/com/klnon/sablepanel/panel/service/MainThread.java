...
final class MainThread {
    ...
    static <T> CompletableFuture<T> submit(MinecraftServer server, Callable<T> task) {
    ...
    static <T> T on(MinecraftServer server, long timeoutSeconds, Callable<T> task) throws Exception {
    ...
    static <T> T onUntilComplete(MinecraftServer server, Callable<T> task) throws Exception {
        if (server.isSameThread()) return task.call();
