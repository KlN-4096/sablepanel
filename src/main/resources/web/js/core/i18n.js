...
const T = {
  ...
  detachedPartial:'残骸没有全部清掉(详见刚才的失败提示,通常是有内容不同的磁盘副本,需要先在「去重副本」里处理)。\n\n剩下的残骸仍会被常驻加载连坐 —— 挂票要去几千格外同步拉区块,实测会把主线程卡死并崩服。\n\n仍然继续常驻加载?',
  ...
  membersTitle:(n)=>`依赖组成员 (${n})`,
  membersFold:'收起成员列表', membersUnfold:'展开成员列表', membersRail:(n)=>`成员 ${n}`,
  statBlocks:'方块总数', statSizeL:'尺寸 W×H×D', statMembers:'组成员', statCostL:'逻辑开销 ms/t',
  bodyGone:'该物理体已不存在',
};
const MANUAL = {
  zh: [
    ...
    {k:'operations',label:'操作',sections:[
      {h:'日常操作',body:'<ul><li><b>重扫</b>：立即重建磁盘索引；后台也会每 120 秒自动扫描。</li><li><b>传送</b>：坐标指结构包围盒的<b>底面中心</b>（目的 y=100 表示结构底部落在 y=100）。未加载体会先强制加载；孤儿会先尝试连同依赖一起收养。</li><li><b>常驻加载</b>：只让完整依赖组保持加载并自然运行，不清速度、不固定物理、不暂停 tick；取消后保存当前状态并卸载到磁盘。</li><li><b>暂停物理</b>：先用引擎固定约束锁住完整依赖组，再清除全部线速度和角速度；方块实体仍正常 tick。</li><li><b>暂停 tick</b>：只停止完整依赖组的方块实体 tick，物理仍自然运行。</li><li><b>收养</b>：重建 holding 指针并接回加载管线，不删除或改写原有物理体条目。</li><li><b>删除</b>：按完整依赖组执行，严格验收成功后才提交回收站备份。</li><li><b>批量操作</b>：勾选会跨筛选保留；批量删除仍会按依赖组去重并逐组验收。</li></ul>'},
    ...
    {k:'recycle',label:'回收站',sections:[
      {h:'何时进入回收站',body:'<p>删除前会暂存完整依赖组备份；磁盘条目和 holding 指针通过删除后验收时记为可恢复。删除或自动回滚中断时，完整备份会记为删除未完成，不再隐藏。回收站不会自动清除任何组，容量不足时请在列表中多选并彻底删除。</p>'},
      {h:'恢复与容量',body:'<ul><li>恢复始终恢复完整依赖组，并在保存的维度和位置重新加载；旧版本不会覆盖世界中已有的同 UUID 物理体。</li><li><b>可恢复</b>表示删除成功且备份可用；最新版本的<b>删除未完成</b>会先清除同 UUID 残留再从快照重建整组；<b>已恢复</b>表示该记录已经成功执行过恢复。</li><li>容量按实际备份文件计数。面板不会自动淘汰任何回收组；达到或超过上限时，新删除会在执行前被拒绝，需人工多选并彻底删除。</li></ul>'}
    ]},
