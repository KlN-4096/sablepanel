...
const T = {
  ...
  selPause:(n)=>`暂停所选 (${n})`, selResume:(n)=>`恢复所选 (${n})`,
      pauseTip:'暂停物理 = 钉住不让动(引擎固定约束,同物理手杖)。\n结构里的东西照常运转:Create 机器转、流体流、漏斗吸。\n\n用于:体被撞跑 / 自己往下掉 / 想固定住当展品。\n只对点名的这一个体生效,重启后保持。',
  freezeBody:'❄ 暂停 tick', thawBody:'☀ 恢复 tick', frozenTag:'已暂停 tick(整组停机)',
  selFreeze:(n)=>`暂停 tick (${n})`, selThaw:(n)=>`恢复 tick (${n})`,
  freezeTip:'暂停 tick = 让结构停机(拦掉全部方块实体 tick)。\nCreate 机器、流体网络、漏斗全部不动,并连带锁住物理。\n\n用于:整组太重,留在内存里就把主线程压垮。\n按整个依赖组生效(点一个体连坐全组),常驻加载会自动开启,重启后保持。',
  thawT:'恢复 tick',
  ...
  copyEvidenceStray:'运行证据指向残缺或未知条目，建议先做一致性检查',
