...
function renderJobs(){
  ...
  host.innerHTML = `<div class="jobRow jobHead">
      <span>${T.jobColTime}</span><span>${T.jobColOp}</span><span>${T.jobColTarget}</span>
      <span>${T.jobColResult}</span><span style="text-align:right">${T.jobColMs}</span><span>${T.jobColMsg}</span>
    </div>` + rows.map(job => {
    const when = fmtTime(job.started_at || job.queued_at);
